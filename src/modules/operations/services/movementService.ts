  ...
  async getMovements(): Promise<Movement[]> {
    try {
      ...
      return data.map(movement => ({
        ...
        actualized: movement.actualized || false,
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          nominated_date: movement.nominatedDate ? movement.nominatedDate.toISOString().split('T')[0] : null,
          nomination_valid_date: movement.nominationValidDate ? movement.nominationValidDate.toISOString().split('T')[0] : null,
          cash_flow_date: movement.cashFlowDate ? movement.cashFlowDate.toISOString().split('T')[0] : null,
          vessel_name: movement.vesselName,
          loadport: movement.loadport,
          disport: movement.disport,
          inspector: movement.inspector,
          bl_date: movement.blDate ? movement.blDate.toISOString().split('T')[0] : null,
          bl_quantity: movement.blQuantity,
          actualized: movement.actualized || false,
          actualized_date: movement.actualizedDate ? movement.actualizedDate.toISOString().split('T')[0] : null,
      ...
      return {
        ...
        actualized: data.actualized || false,
