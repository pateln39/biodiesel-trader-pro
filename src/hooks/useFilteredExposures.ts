    ...
    try {
      ...
      physicalTrades.forEach(trade => {
        ...
        trade.legs.forEach(leg => {
          ...
          // PHYSICAL EXPOSURES - Always use loading period start month
          if (leg.mtmFormula && leg.mtmFormula.exposures && leg.mtmFormula.exposures.physical) {
            ...
            // IMPORTANT CHANGE: We're no longer checking for explicit monthly distributions
            // Instead, we always calculate based on loading period start
            console.log(`Using loading period start for physical exposure distribution`);
            
            // Process each instrument in the physical exposure
            Object.entries(leg.mtmFormula.exposures.physical).forEach(([instrument, value]) => {
              ...
              if (leg.loadingPeriodStart) {
                ...
                // Fallback to pricing period if no loading period start
          ...
          // PRICING EXPOSURES - No changes needed, keep existing logic
