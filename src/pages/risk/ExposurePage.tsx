            ...
            <CardContent className="p-0 overflow-auto">
              ...
              minWidth: "100%"
                  ...
                  <Table className="border-collapse">
                    <TableHeader>
                      <TableRow className="bg-gradient-to-br from-brand-navy/75 via-brand-navy/60 to-brand-lime/25 border-b-[1px] border-black">
                        <TableHead rowSpan={2} className="border-r-[3px] border-brand-lime/30 border-b-[1px] border-black text-left p-1 font-bold text-white text-xs bg-brand-navy sticky left-0 z-10">
                          Month
                        </TableHead>
                      ...
                      return <TableHead key={category} colSpan={colSpan} className={`text-center p-1 font-bold text-white text-xs border-b-[1px] ${catIndex < orderedVisibleCategories.length - 1 ? 'border-r-[3px] border-brand-lime/30' : ''} border-black`}>
                              {category}
                            </TableHead>;
                    })}
                      </TableRow>
                      ...
                      if (category === 'Exposure') {
                        ...
                        categoryProducts.forEach((product, index) => {
                          headers.push(<TableHead key={`${category}-${product}`} className={`text-right p-1 text-xs whitespace-nowrap border-t-0 border-r-[3px] border-brand-lime/30 ${getExposureProductBackgroundClass(product)} text-white font-bold`}>
                                  {formatExposureTableProduct(product)}
                                </TableHead>);
                          if (index === ucomeIndex && shouldShowBiodieselTotal) {
                            headers.push(<TableHead key={`${category}-biodiesel-total`} className={`text-right p-1 text-xs whitespace-nowrap border-t-0 border-r-[3px] border-brand-lime/30 ${getCategoryColorClass(category)} text-white font-bold`}>
                                    Total Biodiesel
                                  </TableHead>);
                          }
                        });
                        if (shouldShowPricingInstrumentTotal) {
                          headers.push(<TableHead key={`${category}-pricing-instrument-total`} className={`text-right p-1 text-xs whitespace-nowrap border-t-0 border-r-[3px] border-brand-lime/30 ${getExposureProductBackgroundClass('', false, true)} text-white font-bold`}>
                                  Total Pricing Instrument
                                </TableHead>);
                        }
                        if (shouldShowTotalRow) {
                          headers.push(<TableHead key={`${category}-total-row`} className={`text-right p-1 text-xs whitespace-nowrap border-t-0 ${getExposureProductBackgroundClass('', true)} ${catIndex < orderedVisibleCategories.length - 1 ? 'border-r-[3px] border-brand-lime/30' : ''} text-white font-bold`}>
                                  Total Row
                                </TableHead>);
                        }
                        return headers;
                      } else {
                        return categoryProducts.map((product, index) => <TableHead key={`${category}-${product}`} className={`text-right p-1 text-xs whitespace-nowrap border-t-0 ${getCategoryColorClass(category)} ${index === categoryProducts.length - 1 && catIndex < orderedVisibleCategories.length - 1 ? 'border-r-[3px] border-brand-lime/30' : ''} ${index > 0 ? 'border-l-[0px]' : ''} text-white font-bold`}>
                                {formatExposureTableProduct(product)}
                              </TableHead>);
                      }
                    })}
                    ...
                    <TableBody>
                      {exposureData.map(monthData => <TableRow key={monthData.month} className="bg-gradient-to-br from-brand-navy/75 via-brand-navy/60 to-brand-lime/25">
                          <TableCell className="font-medium border-r-[3px] border-brand-lime/30 border-black text-xs sticky left-0 z-10 bg-brand-navy text-white">
                            {monthData.month}
                          </TableCell>
                      ...
                      <TableRow className="bg-gradient-to-br from-brand-navy/75 via-brand-navy/60 to-brand-lime/25 border-t-[1px] border-black text-white font-bold">
                        <TableCell className="border-r-[3px] border-brand-lime/30 border-black text-xs p-1 sticky left-0 bg-gray-700 z-10 text-white">
                          Total
                        </TableCell>
